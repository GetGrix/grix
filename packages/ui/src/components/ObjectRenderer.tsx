...
export function ObjectRenderer({
  ...
  const scaledFontSize = (baseSize: number) =>
    Math.round(baseSize * visualSettings.fontScale);
  ...
  const [touchTangentInfo, setTouchTangentInfo] = useState<{
    circleId: string;
    point: Point;
    slope: number | null;
  } | null>(null);
  ...
  const [triangleHoverPoint, setTriangleHoverPoint] = useState<Point | null>(
    null
  );

  // Track function hover for coordinate display
  const [hoveredFunction, setHoveredFunction] = useState<string | null>(null);
  const [functionHoverPoint, setFunctionHoverPoint] = useState<Point | null>(
    null
  );
  ...
  const renderObject = (obj: MathObject) => {
    ...
    switch (obj.type) {
      case "ray":
        ...
        return (
          <g key={obj.id}>
            ...
            {visualSettings.showCoordinateProjections &&
              Math.abs(obj.properties.startPoint.x) < 0.001 &&
              Math.abs(obj.properties.startPoint.y) < 0.001 && (
                <>
            ...
            {/* Start point handle with larger invisible hit area */}
            <g>
              <circle
                cx={start.x}
                cy={start.y}
              ...
              <circle
                cx={start.x}
                cy={start.y}
                r={Math.max(12, touchTargetSize / 2)}
                fill="transparent"
                style={{ cursor: "move" }}
            ...
            {/* End point handle with larger invisible hit area */}
            <g>
              <circle
                cx={end.x}
                cy={end.y}
            ...
            {!(
              ...
              hoveredEndpoint === `${obj.id}-start` && (
                ...
                >
                  ({formatCoordinate(obj.properties.startPoint.x, gridSize)},{" "}
                  {formatCoordinate(obj.properties.startPoint.y, gridSize)})
                </text>
              )}
              ...
              <text
                ...
                y={(() => {
                  ...
                  const isFromOrigin =
                    Math.abs(obj.properties.startPoint.x) < 0.001 &&
                    Math.abs(obj.properties.startPoint.y) < 0.001;
                  return isFromOrigin ? end.y - 10 : end.y - 10;
                })()}
                fontSize={scaledFontSize(10)}
                fontWeight="500"
                fill={isSelected ? "#1D4ED8" : "#2563eb"}
                ...
                style={{ pointerEvents: "none" }}
              >
                ({formatCoordinate(obj.properties.endPoint.x, gridSize)},{" "}
                {formatCoordinate(obj.properties.endPoint.y, gridSize)})
              </text>
            )}
            ...
            {(() => {
              const dx =
                obj.properties.endPoint.x - obj.properties.startPoint.x;
              const dy =
                obj.properties.endPoint.y - obj.properties.startPoint.y;
            ...
            {(() => {
              ...
              const viewBounds = {
                ...
                bottom:
                  viewport.center.y - canvasSize.height / 2 / viewport.zoom,
              };

              // Extend the line far beyond the viewport
              const maxExtent =
                Math.max(
                  Math.abs(viewBounds.left),
                  Math.abs(viewBounds.right),
                  Math.abs(viewBounds.top),
                  Math.abs(viewBounds.bottom)
                ) * 2;
              ...
              if (Math.abs(endX) > 0.001 && Math.abs(endY) > 0.001) {
                ...
                for (let multiple = 2; multiple <= 50; multiple++) {
                  ...
                  if (
                    Math.abs(equivX - snapX) < snapSize / 10 &&
              ...
              if (originalLength > 0 && visualSettings.showLengthMultiples) {
                for (let multiple = 2; multiple <= 5; multiple++) {
                  ...
                  if (
                    screenPos.x >= -50 &&
                    screenPos.x <= canvasSize.width + 50 &&
                    screenPos.y >= -50 &&
                    screenPos.y <= canvasSize.height + 50
                  ) {
              ...
              return (
                <g>
                    ...
                    (() => {
                      ...
                      if (endX > 0 && endY > 0) {
                        ...
                        const rectWidth = Math.abs(
                          endpointScreen.x - originScreen.x
                        );
                        const rectHeight = Math.abs(
                          endpointScreen.y - originScreen.y
                        );
                        ...
                        return (
                          <g>
                            ...
                            <text
                              ...
                              opacity="0.6"
                            >
                              {formatCoordinate(endY, gridSize)} ×{" "}
                              {formatCoordinate(endX, gridSize)} ={" "}
                              {formatCoordinate(area, gridSize)}
                            </text>
                          </g>
                        );
                      }
                  ...
                  {visualSettings.showDivisionMultiples &&
                    visualSettings.showDivisionAnswer &&
                    (() => {
                      ...
                      for (
                        ...
                        const isAnswerLine =
                          Math.abs(yValue - divisionAnswer) < 0.01;

                        // Add shaded rectangle if area rectangle is off
                        if (!visualSettings.showAreaRectangle) {
                          const rectHeight =
                            Math.abs(yScreen - originScreen.y) / multiple;
                          const rectWidth = Math.abs(
                            xOneScreen.x - originScreen.x
                          );
                  ...
                  {visualSettings.showDistanceMarkers &&
                    (() => {
                      ...
                      if (lineLength > 0) {
                        ...
                        radiiToShow.forEach(({ radius, isUnit }, index) => {
                          ...
                          if (radiusInScreen >= 15 && radiusInScreen <= 5000) {
                            ...
                            if (Math.abs(lineAngle) > 0.05) {
                              ...
                              const arcPath = `M ${
                                originScreen.x + radiusInScreen
                              },${
                                originScreen.y
                              } A ${radiusInScreen},${radiusInScreen} 0 ${largeArcFlag},${sweepFlag} ${
                                originScreen.x +
                                radiusInScreen * Math.cos(lineAngle)
                              },${
                                originScreen.y -
                                radiusInScreen * Math.sin(lineAngle)
                              }`;

                              radialMarkers.push(
                                <path
                                  key={`radial-${obj.id}-${radius.toFixed(
                                    3
                                  )}-${lineAngle.toFixed(3)}-${index}`}
                                  d={arcPath}
                                  fill="none"
                                  stroke={isSelected ? "#1D4ED8" : "#2563eb"}
                        ...
                        const distancePointScreen = worldToScreen({
                          x: lineLength,
                          y: 0,
                        });
                        ...
                        radialMarkers.push(
                          <g key={`distance-${obj.id}`}>
                            ...
                            <path
                              d={`M ${distancePointScreen.x},${
                                originScreen.y
                              } L ${distancePointScreen.x - 4},${
                                originScreen.y + 8
                              } L ${distancePointScreen.x + 4},${
                                originScreen.y + 8
                              } Z`}
                  ...
                  {visualSettings.showAngleArc &&
                    (() => {
                      ...
                      if (Math.abs(endX) > 0.05 || Math.abs(endY) > 0.05) {
                        ...
                        const arcPath = `M ${originScreen.x + arcRadius},${
                          originScreen.y
                        } A ${arcRadius},${arcRadius} 0 ${largeArcFlag},0 ${
                          originScreen.x + arcRadius * Math.cos(endAngle)
                        },${originScreen.y - arcRadius * Math.sin(endAngle)}`;

                        // Position text at the middle of the arc
                        const textAngle = angle / 2;
                        const textRadius = arcRadius * 0.7;
                        const textX =
                          originScreen.x + textRadius * Math.cos(textAngle);
                        const textY =
                          originScreen.y - textRadius * Math.sin(textAngle);
                  ...
                  {visualSettings.showEquivalentFractions &&
                    equivalentPoints.map((point, index) => {
                      ...
                      const shouldSkipForDivision =
                        isAtXOne && visualSettings.showDivisionAnswer;

                      // Don't show green circle if it's at the same location as the line endpoint (which already has blue fraction label)
                      const isAtEndpoint =
                        Math.abs(point.world.x - endX) < 0.1 &&
                        Math.abs(point.world.y - endY) < 0.1;
      ...
      case "rectangle":
        ...
        const bottomRight = {
          x: obj.properties.x + obj.properties.width,
          y: obj.properties.y,
        };
        const topLeftWorld = {
          x: obj.properties.x,
        ...
        return (
          <g key={obj.id}>
            ...
            <rect
              ...
              height={rectHeight}
              fill={
                isSelected ? "rgba(34, 197, 94, 0.3)" : "rgba(34, 197, 94, 0.2)"
              }
            ...
            <text
              ...
              opacity="0.8"
            >
              ({formatCoordinate(topLeftWorld.x, gridSize)},{" "}
              {formatCoordinate(topLeftWorld.y, gridSize)})
            </text>
            <text
              ...
              opacity="0.8"
            >
              ({formatCoordinate(topRight.x, gridSize)},{" "}
              {formatCoordinate(topRight.y, gridSize)})
            </text>
            <text
              x={topLeft.x - 10}
              ...
              opacity="0.8"
            >
              ({formatCoordinate(bottomLeft.x, gridSize)},{" "}
              {formatCoordinate(bottomLeft.y, gridSize)})
            </text>
            <text
              x={topLeft.x + rectWidth + 10}
              y={topLeft.y + rectHeight + 15}
              ...
              opacity="0.8"
            >
              ({formatCoordinate(bottomRight.x, gridSize)},{" "}
              {formatCoordinate(bottomRight.y, gridSize)})
            </text>
            ...
            <text
              ...
              opacity="0.9"
            >
              {formatCoordinate(obj.properties.height, gridSize)} ×{" "}
              {formatCoordinate(obj.properties.width, gridSize)} ={" "}
              {formatCoordinate(obj.properties.area, gridSize)}
            </text>
            ...
            {(() => {
              ...
              if (visualSettings.showFactorPairs && area > 1 && area <= 50) {
                ...
                for (let w = 1; w <= Math.sqrt(area); w++) {
                  if (area % w === 0) {
                    const h = area / w;
                    if (w !== width || h !== height) {
                      // Don't show the current rectangle
              ...
              if (visualSettings.showCommutativeProperty && width !== height) {
                ...
                educationalElements.push(
                  <g key="commutative">
                    ...
                    <path
                      d={`M ${flippedRect.x + flippedRect.width + 5},${
                        flippedRect.y + flippedRect.height / 2
                      } L ${topLeft.x - 5},${topLeft.y + rectHeight / 2}`}
                      stroke="#3B82F6"
                      strokeWidth="1"
                      fill="none"
              ...
              if (
                visualSettings.showPrimeComposite &&
                area > 1 &&
                area <= 100
              ) {
                const isPrime =
                  area > 1 &&
                  ![...Array(Math.floor(Math.sqrt(area)) + 1).keys()]
                    .slice(2)
                    .some((i) => area % i === 0);
                  ...
                  educationalElements.push(
                    <g key="lcm">
                      ...
                      >
                        ...
                        {horizontalCount * verticalCount} rectangles
                      </text>
                    </g>
                  );
                }
              ...
              if (
                visualSettings.showDistributiveProperty &&
                width > 2 &&
                height > 1
              ) {
                ...
                educationalElements.push(
                  <g key="distributive">
                    ...
                    <text
                      ...
                      opacity="0.9"
                    >
                      {height} × ({split} + {width - split}) = {height * split}{" "}
                      + {height * (width - split)} = {area}
                    </text>
                  </g>
                );
              }
      ...
      case "circle":
        ...
        const showTangent =
          visualSettings.showTangentLines &&
          ((hoveredCircle === obj.id && mousePosition) ||
            (touchTangentInfo && touchTangentInfo.circleId === obj.id));
        ...
        if (showTangent) {
          ...
          if (touchTangentInfo && touchTangentInfo.circleId === obj.id) {
            ...
            worldMouse = {
              x:
                (mousePosition.x - canvasSize.width / 2) / viewport.zoom +
                viewport.center.x,
              y:
                -(mousePosition.y - canvasSize.height / 2) / viewport.zoom +
                viewport.center.y,
            };
          }
          ...
          if (worldMouse) {
            ...
            const tangentPoint = {
              x:
                obj.properties.center.x +
                obj.properties.radius * Math.cos(angle),
              y:
                obj.properties.center.y +
                obj.properties.radius * Math.sin(angle),
            };

            // Calculate tangent slope (perpendicular to radius)
            const radiusSlope = dy / dx;
            const tangentSlope =
              isFinite(radiusSlope) && radiusSlope !== 0
                ? -1 / radiusSlope
                : null;

            // Calculate tangent line endpoints (extend beyond visible area)
            const lineLength =
              Math.max(canvasSize.width, canvasSize.height) / viewport.zoom;
            const tangentStart = {
          ...
          <g key={obj.id}>
              ...
              onTouchStart={(e) => {
                if (visualSettings.showTangentLines) {
                  ...
                  if (rect && e.touches[0]) {
                    ...
                    const worldTouch = {
                      x:
                        (touchPoint.x - canvasSize.width / 2) / viewport.zoom +
                        viewport.center.x,
                      y:
                        -(touchPoint.y - canvasSize.height / 2) /
                          viewport.zoom +
                        viewport.center.y,
                    };
                    ...
                    const radiusSlope = dy / dx;
                    const tangentSlope = isFinite(radiusSlope)
                      ? -1 / radiusSlope
                      : null;
            ...
            {tangentInfo && (
              <g>
                ...
                {(tangentInfo.slope === null ||
                  !isFinite(tangentInfo.slope)) && (
                  <text
                    x={worldToScreen(tangentInfo.point).x + 15}
                    y={worldToScreen(tangentInfo.point).y - 10}
                    fontSize={scaledFontSize(10)}
      ...
      case "triangle":
        ...
        const enhancedProps = (() => {
          ...
          if (Math.abs(sideA - sideB) < 0.1 && Math.abs(sideB - sideC) < 0.1) {
            ...
            Math.abs(sideA - sideB) < 0.1 ||
            Math.abs(sideB - sideC) < 0.1 ||
            Math.abs(sideA - sideC) < 0.1
          ) {
          ...
          if (
            Math.abs(angles[0] - 30) < tolerance &&
            ...
            Math.abs(angles[1] - 45) < tolerance &&
            Math.abs(angles[2] - 90) < tolerance
          ) {
            isSpecial = true;
        ...
        const sohcahtoaInfo = (() => {
          if (!visualSettings.showSOHCAHTOA || !enhancedProps.isRight)
            return null;
          ...
          const referenceAngle = acuteAngles.reduce((max, angle) =>
            angle.value > max.value ? angle : max
          );

          const rightVertex = rightAngle.index;
          const refVertex = referenceAngle.index;
          const thirdVertex = [0, 1, 2].find(
            (i) => i !== rightVertex && i !== refVertex
          )!;
        ...
        return (
          <g key={obj.id}>
            ...
            <path
              ...
              onMouseMove={(e) => {
                if (
                  visualSettings.showTrigValues &&
                  hoveredTriangle === obj.id
                ) {
                  const rect =
                    e.currentTarget.ownerSVGElement?.getBoundingClientRect();
                  if (rect) {
            ...
            {sohcahtoaInfo && (
              <g>
                ...
                <g>
                  ...
                  <text
                    x={
                      (vertices[sohcahtoaInfo.hypotenuseStart].x +
                        vertices[sohcahtoaInfo.hypotenuseEnd].x) /
                      2
                    }
                    y={
                      (vertices[sohcahtoaInfo.hypotenuseStart].y +
                        vertices[sohcahtoaInfo.hypotenuseEnd].y) /
                        2 -
                      15
                    }
                    fontSize={scaledFontSize(11)}
                  ...
                  <text
                    x={
                      (vertices[sohcahtoaInfo.oppositeStart].x +
                        vertices[sohcahtoaInfo.oppositeEnd].x) /
                        2 +
                      15
                    }
                    y={
                      (vertices[sohcahtoaInfo.oppositeStart].y +
                        vertices[sohcahtoaInfo.oppositeEnd].y) /
                      2
                    }
                    fontSize={scaledFontSize(10)}
                    fontWeight="600"
                  ...
                  <text
                    x={
                      (vertices[sohcahtoaInfo.adjacentStart].x +
                        vertices[sohcahtoaInfo.adjacentEnd].x) /
                      2
                    }
                    y={
                      (vertices[sohcahtoaInfo.adjacentStart].y +
                        vertices[sohcahtoaInfo.adjacentEnd].y) /
                        2 +
                      20
                    }
                    fontSize={scaledFontSize(10)}
                  ...
                  >
                    Reference Angle:{" "}
                    {sohcahtoaInfo.referenceAngle.value.toFixed(1)}°
                  </text>
                </g>
              </g>
            )}
              ...
              <g>
                {(() => {
                  ...
                  const legA = Math.sqrt(
                    (rightVertex.x - vertex1.x) ** 2 +
                      (rightVertex.y - vertex1.y) ** 2
                  );
                  const legB = Math.sqrt(
                    (rightVertex.x - vertex2.x) ** 2 +
                      (rightVertex.y - vertex2.y) ** 2
                  );
                  const hypotenuse = Math.sqrt(
                    (vertex1.x - vertex2.x) ** 2 + (vertex1.y - vertex2.y) ** 2
                  );
                  ...
                  const testPointA1 = {
                    x: rightVertex.x + legA * perpA1.x * 0.1,
                    y: rightVertex.y + legA * perpA1.y * 0.1,
                  };
                  const testPointA2 = {
                    x: rightVertex.x + legA * perpA2.x * 0.1,
                    y: rightVertex.y + legA * perpA2.y * 0.1,
                  };
                  const perpA = isPointInTriangle(
                    testPointA1,
                    rightVertex,
                    vertex1,
                    vertex2
                  ...
                  const testPointB1 = {
                    x: rightVertex.x + legB * perpB1.x * 0.1,
                    y: rightVertex.y + legB * perpB1.y * 0.1,
                  };
                  const testPointB2 = {
                    x: rightVertex.x + legB * perpB2.x * 0.1,
                    y: rightVertex.y + legB * perpB2.y * 0.1,
                  };
                  const perpB = isPointInTriangle(
                    testPointB1,
                    rightVertex,
                    vertex1,
                    vertex2
                  ...
                  const testPointHyp1 = {
                    x: midHyp.x + hypotenuse * perpHyp1.x * 0.1,
                    y: midHyp.y + hypotenuse * perpHyp1.y * 0.1,
                  };
                  const testPointHyp2 = {
                    x: midHyp.x + hypotenuse * perpHyp2.x * 0.1,
                    y: midHyp.y + hypotenuse * perpHyp2.y * 0.1,
                  };
                  const perpHyp = isPointInTriangle(
                    testPointHyp1,
                    rightVertex,
                    vertex1,
                    vertex2
                  ...
                  const squareA = [
                    rightVertex,
                    {
                      x: rightVertex.x + legA * dirA.x,
                      y: rightVertex.y + legA * dirA.y,
                    },
                    {
                      x: rightVertex.x + legA * dirA.x + legA * perpA.x,
                      y: rightVertex.y + legA * dirA.y + legA * perpA.y,
                    },
                    {
                      x: rightVertex.x + legA * perpA.x,
                      y: rightVertex.y + legA * perpA.y,
                    },
                  ];
                  ...
                  const squareB = [
                    rightVertex,
                    {
                      x: rightVertex.x + legB * dirB.x,
                      y: rightVertex.y + legB * dirB.y,
                    },
                    {
                      x: rightVertex.x + legB * dirB.x + legB * perpB.x,
                      y: rightVertex.y + legB * dirB.y + legB * perpB.y,
                    },
                    {
                      x: rightVertex.x + legB * perpB.x,
                      y: rightVertex.y + legB * perpB.y,
                    },
                  ];
                  ...
                  const squareC = [
                    ...
                    vertex2,
                    {
                      x: vertex2.x + hypotenuse * perpHyp.x,
                      y: vertex2.y + hypotenuse * perpHyp.y,
                    },
                    {
                      x: vertex1.x + hypotenuse * perpHyp.x,
                      y: vertex1.y + hypotenuse * perpHyp.y,
                    },
                  ];
                  ...
                  return (
                    <g>
                      ...
                      {(() => {
                        ...
                        const allSquarePoints = [
                          ...squareAScreen,
                          ...squareBScreen,
                          ...squareCScreen,
                        ];
                        const minX = Math.min(
                          ...allSquarePoints.map((p) => p.x)
                        );
                        const maxX = Math.max(
                          ...allSquarePoints.map((p) => p.x)
                        );
                        const minY = Math.min(
                          ...allSquarePoints.map((p) => p.y)
                        );
                        const maxY = Math.max(
                          ...allSquarePoints.map((p) => p.y)
                        );
            ...
            {visualSettings.showTriangleAltitudes && (
              <g>
                {worldVertices.map((vertex, i) => {
                  ...
                  const t =
                    ((vertex.x - opposite1.x) * dx +
                      (vertex.y - opposite1.y) * dy) /
                    (length * length);
                  const foot = {
                    x: opposite1.x + t * dx,
                    y: opposite1.y + t * dy,
            ...
            {visualSettings.showTriangleAngles && (
              <g>
                ...
                >
                  Sum:{" "}
                  {(
                    enhancedProps.angleA +
                    enhancedProps.angleB +
                    enhancedProps.angleC
                  ).toFixed(1)}
                  °
                </text>
              </g>
            ...
            {visualSettings.showTriangleClassification && (
              <g>
                ...
                >
                  {enhancedProps.isSpecial
                    ? enhancedProps.specialType
                    : enhancedProps.classification}
                </text>
            ...
            <text
              ...
              y={
                (v0.y + v1.y + v2.y) / 3 +
                (visualSettings.showTriangleAngles ? 35 : 8)
              }
              fontSize={scaledFontSize(10)}
              fontWeight="500"
              fill={isSelected ? "#DC2626" : "#F56565"}
                    ...
                    return (
                      <g>
                        ...
                        <text
                          ...
                          textAnchor="middle"
                        >
                          Angle {closestAngle.label}:{" "}
                          {closestAngle.value.toFixed(1)}°
                        </text>
                        <text
                          x={triangleHoverPoint.x + 80}
      ...
      case "function":
        ...
        const domainStart = worldToScreen({
          x: functionObj.properties.domain.min,
          y: points[0]?.y || 0,
        });
        const domainEnd = worldToScreen({
          x: functionObj.properties.domain.max,
          ...
          <g key={obj.id}>
              ...
              (() => {
                ...
                const evaluateFunction = (
                  ...
                  try {
                    ...
                    const y = func(x);
                    return typeof y === "number" && !isNaN(y) && isFinite(y)
                      ? y
                      : null;
                  } catch (e) {
                    return null;
                ...
                const viewBounds = {
                  left:
                    viewport.center.x - canvasSize.width / 2 / viewport.zoom,
                  right:
                    viewport.center.x + canvasSize.width / 2 / viewport.zoom,
                };
                ...
                if (functionObj.properties.domain.min > viewBounds.left) {
                  ...
                  const startX = Math.max(
                    viewBounds.left,
                    functionObj.properties.domain.min - 6
                  );
                  const step =
                    (functionObj.properties.domain.min - startX) / 15;
                  ...
                  if (leftPoints.length > 1) {
                    const leftPath = leftPoints.reduce(
                      (path, point, idx) =>
                        path +
                        (idx === 0
                          ? `M ${point.x} ${point.y}`
                ...
                if (functionObj.properties.domain.max < viewBounds.right) {
                  ...
                  const endX = Math.min(
                    viewBounds.right,
                    functionObj.properties.domain.max + 6
                  );
                  const step = (endX - functionObj.properties.domain.max) / 15;

                  for (
                    let x = functionObj.properties.domain.max;
                    x <= endX;
                    x += step
                  ) {
                    const y = evaluateFunction(equation, x);
                  ...
                  if (rightPoints.length > 1) {
                    const rightPath = rightPoints.reduce(
                      (path, point, idx) =>
                        path +
                        (idx === 0
                          ? `M ${point.x} ${point.y}`
              ...
              onMouseMove={(e) => {
                if (hoveredFunction === obj.id) {
                  ...
                  if (rect) {
                    ...
                    const worldX =
                      viewport.center.x +
                      (screenX - canvasSize.width / 2) / viewport.zoom;
                    const worldY =
                      viewport.center.y -
                      (screenY - canvasSize.height / 2) / viewport.zoom;
                    ...
                    for (const point of points) {
                      ...
                      const distance = Math.sqrt(
                        (screenX - screenPoint.x) ** 2 +
                          (screenY - screenPoint.y) ** 2
                      );
                      if (distance < minDistance) {
                        minDistance = distance;
                        closestPoint = point;
            ...
            <path
              ...
              stroke={
                isSelected
                  ? "#1D4ED8"
                  : functionObj.properties.color || "#2563eb"
              }
              strokeWidth={
                isSelected ? 3 : functionObj.properties.strokeWidth || 2
              }
              strokeLinecap="round"
              strokeLinejoin="round"
              style={{ pointerEvents: "none" }}
            ...
            {hoveredFunction === obj.id && functionHoverPoint && (
              <g>
                ...
                <text
                  ...
                  textAnchor="middle"
                >
                  ({formatCoordinate(functionHoverPoint.x, gridSize)},{" "}
                  {formatCoordinate(functionHoverPoint.y, gridSize)})
                </text>
                <text
                  x={worldToScreen(functionHoverPoint).x}
                  ...
                  textAnchor="middle"
                >
                  f({formatCoordinate(functionHoverPoint.x, gridSize)}) ={" "}
                  {formatCoordinate(functionHoverPoint.y, gridSize)}
                </text>
                <text
                  x={worldToScreen(functionHoverPoint).x}
  ...
  return (
    <>
      ...
      <g className="objects">{objects.map(renderObject)}</g>
    </>
  );
}
